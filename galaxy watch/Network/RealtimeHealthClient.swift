...
class RealtimeHealthClient: ObservableObject {
    ...
    init(serverHost: String = "192.168.1.100", serverPort: Int = 3000) {
        self.deviceId = UIDevice.current.identifierForVendor?.uuidString ?? "ios_device_\(Date().timeIntervalSince1970)"
    ...
    private func handleHealthDataUpdate(_ json: [String: Any]) {
        guard let data = json["data"] as? [String: Any],
              let healthData = data["data"] as? [String: Any] else { return }
        ...
        print("Health data updated: HR=\(updatedData.currentHeartRate), Steps=\(updatedData.currentSteps)")
    }
    ...
    private func startReconnectTimer() {
        ...
        print("Reconnecting in \(delay) seconds (attempt \(reconnectAttempts + 1)/\(maxReconnectAttempts))")
    }
