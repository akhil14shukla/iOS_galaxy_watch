...
class HybridSyncManager: ObservableObject {
    ...
    private func collectPendingData(since remoteSyncState: SyncState? = nil) async -> HealthDataBatch {
    ...
    func requestAuthorization() {
        ...
        healthStore.requestAuthorization(toShare: typesToWrite, read: typesToRead) { [weak self] (success: Bool, error: Error?) in
    ...
    func saveHeartRate(_ data: HeartRateData) async {
        ...
        let quantity = HKQuantity(unit: HKUnit.count().unitDivided(by: .minute()), doubleValue: data.value)
    ...
    private func saveWorkout(_ data: HybridWorkoutData) async {
        ...
        do {
            try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, Error>) in
                builder.beginCollection(withStart: data.startTime) { success, error in
                    if let error = error {
                        ...
                        continuation.resume(throwing: NSError(domain: "HealthKit", code: 0, userInfo: [NSLocalizedDescriptionKey: "Failed to begin workout collection"]))
                    }
                }
            }
            ...
            if data.totalDistance > 0 {
                ...
                let distanceQuantity = HKQuantity(unit: HKUnit.meter(), doubleValue: data.totalDistance)
                ...
                try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, Error>) in
                    builder.add([distanceSample]) { success, error in
                        if let error = error {
                            ...
                            continuation.resume(throwing: NSError(domain: "HealthKit", code: 0, userInfo: [NSLocalizedDescriptionKey: "Failed to add distance sample"]))
                        }
                    }
                }
            }
            ...
            if data.totalCalories > 0 {
                ...
                let calorieQuantity = HKQuantity(unit: HKUnit.kilocalorie(), doubleValue: data.totalCalories)
                ...
                try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, Error>) in
                    builder.add([calorieSample]) { success, error in
                        if let error = error {
                            ...
                            continuation.resume(throwing: NSError(domain: "HealthKit", code: 0, userInfo: [NSLocalizedDescriptionKey: "Failed to add calorie sample"]))
                        }
                    }
                }
            }
            ...
            try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, Error>) in
                builder.endCollection(withEnd: data.endTime) { success, error in
                    if let error = error {
                        ...
                        continuation.resume(throwing: NSError(domain: "HealthKit", code: 0, userInfo: [NSLocalizedDescriptionKey: "Failed to end workout collection"]))
                    }
                }
            }
            ...
            let workout = try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<HKWorkout, Error>) in
                builder.finishWorkout { workout, error in
                    if let error = error {
                        ...
                        continuation.resume(throwing: NSError(domain: "HealthKit", code: 0, userInfo: [NSLocalizedDescriptionKey: "Failed to finish workout"]))
                    }
                }
            }
    ...
    private func saveWorkoutRoute(_ route: [HybridWorkoutData.LocationPoint], for workout: HKWorkout) async {
            ...
            try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, Error>) in
                routeBuilder.insertRouteData(locations) { success, error in
                    if let error = error {
                        ...
                        continuation.resume(throwing: NSError(domain: "HealthKit", code: 0, userInfo: [NSLocalizedDescriptionKey: "Failed to insert route data"]))
                    }
                }
            }
            
            let _ = try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<HKWorkoutRoute, Error>) in
                routeBuilder.finishRoute(with: workout, metadata: [:]) { (workoutRoute: HKWorkoutRoute?, error: Error?) in
                    if let error = error {
                        continuation.resume(throwing: error)
                    } else if let workoutRoute = workoutRoute {
                        continuation.resume(returning: workoutRoute)
                    } else {
                        continuation.resume(throwing: NSError(domain: "HealthKit", code: 0, userInfo: [NSLocalizedDescriptionKey: "Failed to finish route"]))
                    }
                }
            }
